...
Route::get('/produk/review/{id}','user\ProdukController@ShowAllreview')->name('user.produk.review');
...
Route::post('/forgot-passwordResp', function (Request $request) {
    $request->validate(['email' => 'required|email']);

    $status = Password::sendResetLink(
        $request->only('email')
    );
    return view('auth.responResetPassword');
...
Route::group(['middleware' => ['auth','checkRole:buyer']],function(){
    ...
    Route::post('/akun/simpan','user\AkunController@simpan');
    Route::post('/akun/update','user\AkunController@update')->name('user.akun.update');
    ...
    Route::get('/order/invoice','user\OrderController@invoice')->name('user.order.invoice');
    ...
    Route::get('/produk/review/tambah/{id}','user\ReviewController@TambahReview')->name('user.review.tambah');
    Route::post('/produk/review/submit/{id}','user\ReviewController@store')->name('user.review.submit');
});

Route::get('/secret/c-level-only', function () {

    if (! Gate::allows('c-level-only')) {
        abort(403);
    }
...
Route::get('/secret/c-level-only', function () {
    ...
    if (! Gate::allows('finance-team-only')) {
        abort(403);
    }
...
Route::get('/email', 'EmailController@create')->name('get.email');
Route::get('/email', 'EmailController@index')->name('get.invoice');
